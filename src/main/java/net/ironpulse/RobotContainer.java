...
public class RobotContainer {
        ...
        private void configureKeyBindings() {
                                ...
                                .applyRequest(
                                                () -> drive
                                                                .withVelocityX(Utils.sign(-driverController.getLeftY())
                                                                                * xLimiter.calculate(Math.abs(
                                                                                                driverController.getLeftY()))
                                                                                * maxSpeed.magnitude())
                                                                .withVelocityY(
                                                                                Utils.sign(-driverController.getLeftX())
                                                                                                * maxSpeed.magnitude()
                                                                                                * yLimiter.calculate(
                ...
                operatorController.y().whileTrue(new LongShotCommand(shooterSubsystem, indicatorSubsystem,
                                swerveSubsystem, driverController));

                // operatorController.pov(0).whileTrue(Commands.runOnce(() -> {
                // Constants.ShooterConstants.speakerArmOffset =
                // Constants.ShooterConstants.speakerArmOffset.plus(Degrees.of(0.5));
                // }));
                // operatorController.pov(0).whileTrue(Commands.runOnce(() -> {
                // Constants.ShooterConstants.speakerArmOffsetNear =
                // Constants.ShooterConstants.speakerArmOffsetNear.plus(Degrees.of(0.5));
                // }));
                // operatorController.pov(180).whileTrue(Commands.runOnce(() -> {
                // Constants.ShooterConstants.speakerArmOffset =
                // Constants.ShooterConstants.speakerArmOffset.minus(Degrees.of(0.5));
                // }));
                // operatorController.pov(90).whileTrue(Commands.runOnce(() -> {
                // Constants.ShooterConstants.speakerArmOffsetNear =
                // Constants.ShooterConstants.speakerArmOffsetNear.minus(Degrees.of(0.5));
                // }));
                // driverController.y().whileTrue(Commands.runOnce(() -> {
                // Constants.ShooterConstants.speakerArmOffsetFar =
                // Constants.ShooterConstants.speakerArmOffsetFar.plus(Degrees.of(0.5));
                // }));
                // driverController.a().whileTrue(Commands.runOnce(() -> {
                // Constants.ShooterConstants.speakerArmOffsetFar =
                // Constants.ShooterConstants.speakerArmOffsetFar.minus(Degrees.of(0.5));
                // }));
        ...
        private void resetOdometryWithAutoName(String autoName) {
                ...
                swerveSubsystem.seedFieldRelative(Utils.flip() ? GeometryUtil.flipFieldPose(startPose) : startPose);
        }
